...
pyplot.show()
normBetaPrice = empiricalBetaPrice/weightList
...
pyplot.show()
normBetaTotal = empiricalBetaTotal/weightList
...
NewBenchmark = numpy.array([weightList[item] * PriceBenchmark[item] for item in range(T*(N-1))])
...
pyplot.show()
normResidPrice = residAlphaPrice/weightList
...
NormalizedPrice = (PriceReturns - PriceBenchmark)/weightList
Reg = scipy.stats.linregress(PriceBenchmark, NormalizedPrice)
print(Reg)
Res = NormalizedPrice - Reg.slope * PriceBenchmark - Reg.intercept * numpy.ones(T*(N-1))
qqplot(Res, line = 's')
...
NewBenchmark = numpy.array([weightList[item] * TotalBenchmark[item] for item in range(T*(N-1))])
...
pyplot.show()
normResidTotal = residAlphaTotal/weightList
...
NormalizedTotal = (TotalReturns - TotalBenchmark)/weightList
Reg = scipy.stats.linregress(TotalBenchmark, NormalizedTotal)
print(Reg)
Res = NormalizedTotal - Reg.slope * TotalBenchmark - Reg.intercept * numpy.ones(T*(N-1))
qqplot(Res, line = 's')
